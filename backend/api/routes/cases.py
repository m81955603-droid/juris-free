...
async def list_cases(estado: Optional[str]=None, tipo: Optional[str]=None,
    ...
    async with httpx.AsyncClient() as c:
        r = await c.get(f"{SUPABASE_URL}/rest/v1/casos?{params}", headers=sb_user_headers(user))
    if r.status_code not in (200, 206):
        raise HTTPException(502, r.text)
    return {"casos": r.json(), "total": len(r.json())}

@router.post("/")
async def create_case(body: CaseCreate, user: CurrentUser = Depends(get_current_user)):
    payload = body.dict()
    ...
    async with httpx.AsyncClient() as c:
        r = await c.post(f"{SUPABASE_URL}/rest/v1/casos", json=payload, headers=sb_user_headers(user))
    if r.status_code not in (200, 201):
        raise HTTPException(502, r.text)
    return {"caso": r.json()[0] if isinstance(r.json(), list) else r.json()}

@router.get("/{caso_id}")
async def get_case(caso_id: str, user: CurrentUser = Depends(get_current_user)):
    if not SUPABASE_URL:
        return {"caso": None, "notas": [], "timeline": [], "demo": True}
    headers = sb_user_headers(user)
    async with httpx.AsyncClient() as c:
        r1 = await c.get(f"{SUPABASE_URL}/rest/v1/casos?id=eq.{caso_id}&select=*", headers=headers)
        r2 = await c.get(f"{SUPABASE_URL}/rest/v1/caso_notas?caso_id=eq.{caso_id}&order=created_at.desc", headers=headers)
        r3 = await c.get(f"{SUPABASE_URL}/rest/v1/caso_timeline?caso_id=eq.{caso_id}&order=fecha.desc", headers=headers)
    casos = r1.json()
...
async def update_case(caso_id: str, body: CaseUpdate, user: CurrentUser = Depends(get_current_user)):
    payload = {k: v for k, v in body.dict().items() if v is not None}
    payload["updated_at"] = datetime.utcnow().isoformat()
    if not SUPABASE_URL: return {"ok": True, "demo": True}
    async with httpx.AsyncClient() as c:
        r = await c.patch(f"{SUPABASE_URL}/rest/v1/casos?id=eq.{caso_id}", json=payload, headers=sb_user_headers(user))
    if r.status_code not in (200, 204): raise HTTPException(502, r.text)
    return {"ok": True}

@router.delete("/{caso_id}")
async def delete_case(caso_id: str, user: CurrentUser = Depends(get_current_user)):
    if not SUPABASE_URL: return {"ok": True, "demo": True}
    async with httpx.AsyncClient() as c:
        r = await c.delete(f"{SUPABASE_URL}/rest/v1/casos?id=eq.{caso_id}", headers=sb_user_headers(user))
    if r.status_code not in (200, 204): raise HTTPException(502, r.text)
    return {"ok": True}

@router.post("/{caso_id}/notas")
async def add_note(caso_id: str, body: NoteCreate, user: CurrentUser = Depends(get_current_user)):
    ...
    async with httpx.AsyncClient() as c:
        r = await c.post(f"{SUPABASE_URL}/rest/v1/caso_notas", json=payload, headers=sb_user_headers(user))
